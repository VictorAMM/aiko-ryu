...
class SimpleIntentAgent {
  ...
  async generateCLevelPrompt(intent) {
    ...
    try {
      console.log(`[IntentAgent] Generating C-level prompt for complexity: ${intent.complexity}`);
...
async function simpleIntentAgentDemo() {
  console.log('🎯 **Simple Intent Agent Demo**');
  ...
  try {
    ...
    console.log('\n⏳ Initializing SimpleIntentAgent...');
    const intentAgent = new SimpleIntentAgent();
    console.log('✅ Intent agent ready!');
    ...
    for (const prompt of testPrompts) {
      ...
      const intentResult = await intentAgent.analyzeIntent(prompt);
    ...
    const promptResult = await intentAgent.generateCLevelPrompt(testIntent);
    ...
    const orchestrationResult = await intentAgent.orchestrateDevelopment(promptResult.prompt);
    ...
    const loopResult = await intentAgent.monitorInfinityLoops();
    ...
    const approvalResult = await intentAgent.manageConsensusApproval(testPhase);
    ...
    const validationResult = await intentAgent.validateEnterpriseValue(testPhase);
    ...
    const roadmapResult = await intentAgent.generateStrategicRoadmap(testIntent);
    ...
    // Final Summary
    console.log('\n🎉 **Simple Intent Agent Demo Complete!**');
    console.log('=' .repeat(60));
    ...
    console.error('❌ Simple intent agent demo failed:', error);
    process.exit(1);
  }
}

// Run the demo
simpleIntentAgentDemo().catch(error => {
  console.error('💥 Simple intent agent demo failed:', error);
  process.exit(1);
}); 